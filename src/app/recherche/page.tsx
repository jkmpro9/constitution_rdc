...
function SearchContent() {
  ...
  const results = useMemo((): SearchEntry[] => {
    ...
    if (articleMatch) {
      ...
      // Chercher l'article dans le cache — on ne peut pas async ici
      // On va plutôt créer un lien direct
      entries.push({
        type: "article",
        numero: num,
        score: 100,
                        ...
                        <div className="flex items-start gap-3">
                          ...
                          <div className="min-w-0 flex-1">
                            ...
                            <p className="text-xs text-rdc-blue-600 leading-relaxed line-clamp-3">
                              {highlightText(entry.article.contenu, query)}
                            </p>
...
function highlightText(text: string, query: string): React.ReactNode {
  ...
  return parts.map((part, i) =>
    regex.test(part) ? (
      <mark key={i} className="bg-rdc-yellow-200 text-rdc-blue-950 rounded-sm px-0.5">
        {part}
      </mark>
    ) : (
      part
    ...
    <Suspense
      fallback={
        <div className="max-w-3xl mx-auto px-6 py-20 animate-pulse">
          <div className="h-8 w-48 bg-rdc-blue-200 rounded mb-6" />
          <div className="h-12 bg-rdc-blue-100 rounded-xl mb-6" />
          <div className="h-4 w-32 bg-rdc-blue-200 rounded mb-4" />
          <div className="h-24 bg-rdc-blue-100 rounded-xl mb-3" />
          <div className="h-24 bg-rdc-blue-100 rounded-xl mb-3" />
        </div>
      }
    >
